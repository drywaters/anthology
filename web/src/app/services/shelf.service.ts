...
export class ShelfService {
    ...
    list(): Observable<ShelfSummary[]> {
        return this.http
            .get<{ shelves: ShelfSummary[] }>(this.baseUrl)
            .pipe(map((res) => res.shelves));
    }
    ...
    assignItem(shelfId: string, slotId: string, itemId: string): Observable<ShelfWithLayout> {
        return this.http.post<ShelfWithLayout>(`${this.baseUrl}/${shelfId}/slots/${slotId}/items`, {
            itemId,
        });
    }

    removeItem(shelfId: string, slotId: string, itemId: string): Observable<ShelfWithLayout> {
        return this.http.delete<ShelfWithLayout>(
            `${this.baseUrl}/${shelfId}/slots/${slotId}/items/${itemId}`,
        );
    }

    scanAndAssign(shelfId: string, slotId: string, isbn: string): Observable<ScanAndAssignResult> {
        return this.http.post<ScanAndAssignResult>(
            `${this.baseUrl}/${shelfId}/slots/${slotId}/scan`,
            { isbn },
        );
    }
}
